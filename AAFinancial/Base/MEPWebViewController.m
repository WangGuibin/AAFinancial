...
@interface MEPWebViewController ()<WKUIDelegate,WKNavigationDelegate>
...
- (UIProgressView *)progressView{
    if (!_progressView) {
        ...
        _progressView.tintColor = kAppMainColor;
...
- (instancetype)initWithURLString:(NSString *)urlStr navgationBarTitle:(NSString *)title {
    return [self initWithMethodType:MEPWebRequestTypeGET parameters: nil url:urlStr title:title];
}
...
- (instancetype)initWithMethodType:(MEPWebRequestType )requestType
    ...
    if (self) {
        self.navTitle = title;
        if (requestType == MEPWebRequestTypeGET) {
